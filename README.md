...
출처 :
• 한국지능정보사회진흥원(NIA) AI Hub — 「금융상품·서비스 및 소비자 특성 데이터」
• 서울열린데이터광장 — 「서울시 상권분석서비스」 (서울특별시)
• Kaggle — 「Netflix Top 10 Weekly Dataset」 (원자료: Netflix Top 10)

정책 준수 : 본 레포지토리는 AI Hub, 서울열린데이터광장 및 데이터 제공처의 이용 정책을 엄격히 준수하며, 원본 데이터 파일(소비자.csv, 상권 분석 데이터, Netflix 주차별 CSV 등)은 데이터 보호 및 용량 관리 정책상 Git 트래킹에서 제외되어 있습니다.

📋 [1차 프로젝트] 개요 및 목적
Project1 : 초개인화 페르소나 기반 금융 상품 패키징 및 교차 판매(Cross-selling) 전략 도출
• 배경 및 목적 :
  대규모 금융 거래 데이터(38.5만 건)를 활용하여 비지도학습(K=3 K-Means) 기반 초개인화 페르소나를 도출합니다.
  페르소나별 FP-Growth 연관 규칙 및 NetworkX 연결 중심성 그래프 분석을 적용하여 고객 유지율과 LTV를 극대화하는 맞춤형 교차 판매(Cross-selling) 패키징 시나리오를 수립합니다.
...
Project1-3 : AI 기반 OTT 콘텐츠 흥행 트래킹 및 글로벌 수급 의사결정 지원 시스템 기획
• 배경 및 목적 :
  수십 개 국가의 주차별 순위 변동과 시청 데이터가 파편화되어 있어 콘텐츠 수급 담당자의 수급 판단 및 흥행 예측에 많은 시간이 소요되는 문제를 해결합니다.
  글로벌 시청 실적 및 94개국 순위 데이터를 통합 데이터 마트(Star Schema)로 정제하고, SQL 윈도우 함수(LAG, LEAD, 이동평균) 기반의 순위 상승 모멘텀(Momentum) 지표를 산출합니다.
  Random Forest 기반 변수 중요도 분석 및 시계열 잔류 패턴 분류를 통해 글로벌 Top 10 차트 잔류 가능성을 예측하고, LLM 리포트 연동 등 AI 기반 자동 예측 리포트 서비스(AX) 기획의 데이터 기반을 구축합니다.
